# Flatpack rules (for Claude Code)

> Copy this file to the root of your project. Claude Code reads it
> automatically on every session. Replace or extend with your own
> project notes below the `---` line.
...
dependencies, and can be edited later by a different agent (you, in
six months) without falling apart. Full standard:
...
## When to refuse, part two

The user may ask for "just add a dependency" or "fetch this from
the cloud once." Don't. Either the tool stays local, or it stops
being a Flatpack and graduates to Baseplate. There's no middle.

## References

- Full spec: [SPEC.md](https://github.com/ConceptPending/flatpack/blob/main/SPEC.md)
- Generation prompt (the long version): [prompts/generate-flatpack.md](https://github.com/ConceptPending/flatpack/blob/main/prompts/generate-flatpack.md)
